...
- 这里是一个 Conflux Union 群友自发创建的收集 Conflux Union 成员的圣经存放仓库，以便查看各位群友的~~黑历史~~
...
- 在提交发言/表情包时，**请不要提交因为违反CXU服规而被永久封禁的成员的发言**
...
- 在提交发言/表情包时，请注意内容分类
  ...
  1. **昵称**分类
     ...
     假如群里的 **Trirrin** 发表了一个言论或一个表情包，如果你认为有纪念意义，那么请在仓库根目录下创建一个目录，如：`/Trirrin`。随后再将此言论/表情包添加到 `/Trirrin` 这个目录
...
- 提交的图片命名请根据图片内容来进行命名
  ...
  假如图片里的内容是：“**Trirrin：我不是萝莉控**”，那么图片的命名应该为：`我不是萝莉控.png`，并且放置在 `/Trirrin` 目录下
  ...
  > ![《银趴》](./多人运动/《银趴》.png)
  >
  > 根据以上图片的内容，该图片的命名可以为 `银趴.png` 或者加一个书名号 `《银趴》.png`
  > （能准确描述图片中的内容即可）
